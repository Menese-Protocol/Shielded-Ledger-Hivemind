/// DEMO-only token ledger for the public shielded-pool experience — Menese DeFi Team.
///
/// This is deliberately not a production asset: the faucet tops every caller up to 100,000 DEMO,
/// so balances have no monetary value. Unlike the former test fixture, this deployed interface has
/// no arbitrary balance, allowance, fee, decimal, archive, clock, or dedup mutation controls.
/// Every transfer/approval produces canonical ICRC-3 blocks: the "what the node provider sees"
/// pane reads this ledger's public block log, so the transparent side is real rather than mocked.
...
// Migration for the faucet redesign (once-per-principal -> top-up-to-100k): the old
// `faucet_claimed` roster is deliberately dropped — the new faucet is stateless.
(with migration = func(_old : { var faucet_claimed : [Principal] }) : {} { {} })
persistent actor DemoTokenLedger {
  ...
  // ---- demo faucet: tops the caller back up to 100,000 DEMO (8 decimals) and writes a real
  // 1mint block, so the mint is visible on the public ledger like any other entry. Repeatable
  // by design: II principals are stable, and the demo must be re-runnable by the same person.
  let FAUCET_AMOUNT : Nat = 100_000 * 100_000_000;
  public shared ({ caller }) func faucet() : async { #ok : Nat; #err : Text } {
    let account : ICRC2.Account = { owner = caller; subaccount = null };
    let balance = balanceOf(account);
    if (balance >= FAUCET_AMOUNT) return #err("balance-sufficient");
    let amount : Nat = FAUCET_AMOUNT - balance;
    setBalance(account, FAUCET_AMOUNT);
    ignore appendBlock(
      "1mint",
      [("amt", #Nat(amount)), ("to", accountValue(account))],
      0,
      true,
      #Mint({
        to = NnsBlock.accountIdentifier(account.owner, account.subaccount);
